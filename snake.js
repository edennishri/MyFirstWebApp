//finding the canvas from the html
...
//this is for when the user clicks the directions
let xVelocity=0;
let yVelocity=0;
...
//set time out helps the game become faster
    setTimeout(drawGame, 1000/ speed);
} 

function isGameOver(){
    let gameOver = false;
//if game has not started than game isn't over
    if(yVelocity ===0 && xVelocity ===0){
        return false;
    }
    
    //hitting walls cause game over
    if(headX < 0 ){
        gameOver = true;
    }
...
//if the snake head meets his tail the game is over
...
function checkAppleCollision(){
    //if the snake head is in the same position then add to snake
